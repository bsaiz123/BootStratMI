...
extract_metadata_from_columns <- function(data) {
  # Identify intensity columns
  intensity_cols <- grep("^LFQ.intensity", colnames(data), value = TRUE)
  ...
  # Extract treatment (WW or DD)
  metadata$treatment <- ifelse(grepl("WW", metadata$sample), "WW", 
                               ifelse(grepl("DD", metadata$sample), "DD", "Unknown"))
  
  # Extract time point (T## pattern)
  metadata$timepoint <- as.numeric(gsub(".*T(\\d+).*", "\\1", metadata$sample))
  
  # Calculate hours (timepoint * 4)
  metadata$hours <- metadata$timepoint * 4
